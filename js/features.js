...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // -----------------------------------------------------------------
    // 6. Article Like Button Functionality (Dynamic & Persistent)
    // -----------------------------------------------------------------
    const likeButtons = document.querySelectorAll('.btn-like');
    
    // Check if the user has already liked an article from LocalStorage
    function getUserLikes() {
        const likes = localStorage.getItem('user_likes');
        return likes ? JSON.parse(likes) : [];
    }
    
    function saveUserLike(articleId, action) {
        let likes = getUserLikes();
        if (action === 'add' && !likes.includes(articleId)) {
    ...
    // Helper to generate a random starting coordinate based on text, 
    // ensuring identical "fake" values on load when DB is absent.
    function getFallbackCount(articleId) {
        let hash = 0;
        for (let i = 0; i < articleId.length; i++) {
            hash = articleId.charCodeAt(i) + ((hash << 5) - hash);
        }
        return Math.abs(hash % 100) + 10; // returns a number between 10 and 110
    }
    ...
    likeButtons.forEach(btn => {
        ...
        // Setup initial static state based on localStorage for the user 
        // and fake the global likes via a seeded random generator (for local testing).
        // Later, replace with an actual fetch to /api/likes?id={articleId}
        ...
        // Try fetching actual data from potential Vercel API
        fetch(`/api/likes?id=${articleId}`)
            ...
            .then(data => {
                // Determine display count: use actual likes, but if user liked it locally and server is 0
                // (e.g. KV reset), ensure we at least show 1.
                let serverLikes = parseInt(data.likes) || 0;
                if (userLikes.includes(articleId) && serverLikes === 0) {
                    serverLikes = 1;
                }
                countSpan.textContent = serverLikes;
            })
            .catch(() => {
                // Backend not available (running locally or no Vercel KV setup yet)
                // Default to 0, or 1 if the user liked it.
                let displayCount = userLikes.includes(articleId) ? 1 : 0;
        ...
        btn.addEventListener('click', async function(e) {
            ...
            if (isLiked) {
                ...
                saveUserLike(articleId, 'remove');
                
                // Attempt to send to backend API
                fetch('/api/likes', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id: articleId, action: 'decrement' })
                }).catch(() => {}); // silently ignore if API isn't set up
                ...
                icon.classList.add('fa-solid');
                countSpan.textContent = currentCount + 1;
                saveUserLike(articleId, 'add');
                
                // Attempt to send to backend API
                fetch('/api/likes', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id: articleId, action: 'increment' })
                }).catch(() => {}); // silently ignore if API isn't set up
            }
            ...
            if (!isLiked) { // Only animate on like
